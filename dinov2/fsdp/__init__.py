...
try:  # numpy>=1.24 exposes dtype classes under np.dtypes
    ALLOWLISTED_NUMPY_GLOBALS.extend(
        [
...
torch.serialization.add_safe_globals(ALLOWLISTED_NUMPY_GLOBALS)
# Allow sharded tensor state dicts in checkpoints (needed for resume with weights_only=True)
torch.serialization.add_safe_globals([ShardedTensor, Shard, ShardMetadata])
...
class FSDPCheckpointer(Checkpointer):
    def _load_file(self, filename: str):
        """
        Override to ensure sharded tensor classes are allowlisted and to force weights_only=False
        for backwards compatibility with older checkpoints.
        """
        with torch.serialization.safe_globals(
            [ShardedTensor, Shard, ShardMetadata]
        ):
            return torch.load(filename, map_location=torch.device("cpu"), weights_only=False)

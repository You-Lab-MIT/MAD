...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output_list, teacher_out_softmaxed_centered_list):
        ...
        for s in student_output_list:
            student_logits = (s / self.student_temp).float()
            if not torch.isfinite(student_logits).all():
                student_logits = torch.nan_to_num(student_logits, nan=0.0, posinf=1e4, neginf=-1e4)
            lsm = F.log_softmax(student_logits, dim=-1)
            for t in teacher_out_softmaxed_centered_list:
                teacher_probs = torch.nan_to_num(t.float(), nan=0.0, posinf=0.0, neginf=0.0)
                ...
                loss = torch.sum(teacher_probs * lsm, dim=-1)

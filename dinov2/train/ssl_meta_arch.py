...
def _resize_positional_embeddings(state_dict, module):
    ...
    if old_size * old_size != pretrained_token_count or new_size * new_size != current_token_count:
        logger.warning(
            "Unable to reshape positional embeddings from %s to %s; leaving current initialization.",
    ...
    logger.info(
        "Resizing positional embeddings from %s (grid %dx%d) to %s (grid %dx%d)",
        pretrained_pos_embed.shape,
        old_size,
        old_size,
        current_pos_embed.shape,
...
class SSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.student.pretrained_weights:
            ...
            _resize_positional_embeddings(state_dict, student_backbone)
    ...
    def fsdp_synchronize_streams(self):
        ...
        student_backbone_streams = getattr(self.student.backbone, "_streams", None)
        teacher_backbone_streams = getattr(self.teacher.backbone, "_streams", None)
        student_head_streams = getattr(self.student.dino_head, "_streams", None)
        teacher_head_streams = getattr(self.teacher.dino_head, "_streams", None)

        streams_available = all(
            stream is not None
            for stream in (
                student_backbone_streams,
                teacher_backbone_streams,
                student_head_streams,
                teacher_head_streams,
            )
        )

        if not streams_available:
            logger.warning("FSDP streams are unavailable; skipping manual synchronization.")
            self.need_to_synchronize_fsdp_streams = False
            return

        torch.cuda.synchronize()
        shared_streams = teacher_backbone_streams
        self.student.backbone._streams = shared_streams
        self.teacher.backbone._streams = shared_streams
        self.student.dino_head._streams = shared_streams
        self.teacher.dino_head._streams = shared_streams
        self.need_to_synchronize_fsdp_streams = False
